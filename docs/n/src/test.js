...
function setOnClick() {
    ...
    const convb = document.getElementById('conv');
    check.onclick = checkButton;
    button.onclick = copyButton;
    penb.onclick = pen;
    resetb.onclick = reset;
    gob.onclick = gogame;
    convb.onclick = convert;
}

function gogame() {
    var prob = document.getElementById('inp').value;
    prob = prob.replace(/"/,"");
    prob = prob + " ".repeat(36);
    prob = prob.substring(0, 36);
    APP.problem = prob;
    const board = document.getElementById('board');
    board.innerHTML="";
...
function convert() {
    var prob = APP.problem;
    prob = NP6.convert(prob.split(''), RAND).join('');
    APP.problem = prob;
    const board = document.getElementById('board');
    board.innerHTML="";
    makeBoard(board, APP.problem, 6, 3, 2, select);
