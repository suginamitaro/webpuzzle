...
const NumberPlace9 = require('./numberplace9');

const LINESIZE = 9;
const PA9 = 'problemArray9-';
const PUZZLE = {};
PUZZLE.etitle = "numpl9";
PUZZLE.jtitle = "ナンプレ9";
PUZZLE.seed = "";
var APP = {};
const NP9 = new NumberPlace9();
const FNAME = {'L' : "pz.locked9.txt",
               'T' : "pz.tuple9.txt",
               'X' : "pz.xwing9.txt"};
...
function checkButton() {
    ...
    APP.checkCnt++;
    if (NP9.check(sp)) { // check and warninig set
        APP.answer = str;
        success_end();
    } else {
...
function restoreBoard(app) {
    const board = document.getElementById('board');
    makeBoard(board, app.problem, 9, 3, 3, null);
...
function init() {
    ...
    const keystr = APP.keystr + " ";
    const board = document.getElementById('board');
    makeBoard(board, APP.problem, 9, 3, 3, select);
    const key = document.getElementById('key');
...
function randomProblem(problemArray) {
    ...
    //console.log("prstr:" + prstr);
    prstr = prstr.trim();
    prstr = prstr.replace(/"/g,"");
    // if (prstr.length == LINESIZE * LINESIZE + 2) {
    //     prstr = prstr.substring(1, LINESIZE * LINESIZE + 1);
    // }
    console.log("length = " + prstr.length);
    console.log("prstr = " + prstr);
    prstr = NP9.convert(prstr.split(''), RAND).join('');
